...
window.Views = (() => {
  ...
  function statsRange(o) {
    ...
    const skSide = (title, cls, items, side) => {
      ...
      const rows = items.map((x, i) => {
        ...
        const val = '<span class="sk2-val">' + money(x.value) + ' <em>' + pct + '%</em></span>';
        return '<div class="sk2-label' + (side === 'right' ? ' sk2-r' : '') + '" style="flex:' + x.value + '">' +
          (side === 'left' ? dot + name + val : val + name + dot) + '</div>';
      }).join('');
      const lines = items.map((x, i) =>
        '<i class="sk2-line" style="flex:' + x.value + ';background:repeating-linear-gradient(90deg,' + Preset.macaron[i % Preset.macaron.length] + ' 0 4px,transparent 4px 7px)"></i>').join('');
  ...
  function settings() {
    ...
    /* 关于 */
    const aboutBody =
      '<div class="about-name">🧾 轻账单 LiteBill<span class="about-ver">' + ((window.App && App.VERSION) || 'v1.67.0') + '</span></div>' +
